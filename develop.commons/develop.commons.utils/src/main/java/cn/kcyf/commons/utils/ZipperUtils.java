...
public class ZipperUtils {
    private static final Logger log = LoggerFactory.getLogger(ZipperUtils.class);

    /**
     * 制作压缩包
     *
     * @param out        输出流
     * @param fileEntrys 文件过滤，不过滤可以为null
     * @param encoding   源文件名。可以有多个源文件，如果源文件是目录，那么所有子目录都将被包含。
     */
    public static void zip(OutputStream out, List<FileEntry> fileEntrys,
                           String encoding) {
        new ZipperUtils(out, fileEntrys, encoding);
    }
     ...
     */
    public static void zip(OutputStream out, List<FileEntry> fileEntrys) {
        new ZipperUtils(out, fileEntrys, null);
    }
     ...
     * @param encoding   源文件名。可以有多个源文件，如果源文件是目录，那么所有子目录都将被包含。
     */
    protected ZipperUtils(OutputStream out, List<FileEntry> fileEntrys,
                          String encoding) {
